...
const Dashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      // Load workout stats
      const statsResponse = await fetch('http://localhost:8000/api/workouts/stats/summary', {
      ...
      // Load active workout
      const workoutResponse = await fetch('http://localhost:8000/api/workouts/active', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      // Load nutrition data
      try {
        const nutritionResponse = await fetch('http://localhost:8000/api/nutrition/daily-summary', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (nutritionResponse.ok) {
          const nutritionSummary = await nutritionResponse.json();
          setNutritionData({
            todayCalories: nutritionSummary.calories,
            todayProtein: nutritionSummary.protein,
            todayCarbs: nutritionSummary.carbs,
            todayFats: nutritionSummary.fats,
            mealCount: nutritionSummary.meal_count
          });
        }
      } catch (e) {
        console.log('Nutrition API not available yet');
      }

      // Load water data
      try {
        const waterResponse = await fetch('http://localhost:8000/api/nutrition/water/daily', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (waterResponse.ok) {
        ...
        console.log('Water tracking API not available yet');
      }

      // Load posture recommendations
      try {
        const postureResponse = await fetch('http://localhost:8000/api/posture/recommendations', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (postureResponse.ok) {
          const posture = await postureResponse.json();
          setPostureData(prev => ({...prev, recommendations: posture.recommendations}));
        }
      } catch (e) {
        console.log('Posture API not available yet');
      }

      // Load weekly trend
      try {
        const trendResponse = await fetch('http://localhost:8000/api/posture/weekly-trend', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (trendResponse.ok) {
          const trend = await trendResponse.json();
          setWeeklyTrend(trend.trend_data);
        }
      } catch (e) {
        console.log('Posture trend API not available yet');
      }
  ...
  const simulatePostureAnalysis = () => {
    const interval = setInterval(() => {
      ...
      // Generate simulated posture score
  ...
  const logNutrition = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/nutrition/log', {
        method: 'POST',
        headers: {
  ...
  const logWater = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/nutrition/water', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const calculateBMI = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/nutrition/bmi', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
              ...
              <Grid item xs={6}>
                <Card sx={{ 
                  ...
                  <CardContent>
                    ...
                    <Typography variant="caption" sx={{ opacity: 0.7 }}>
                      {nutritionData.mealCount} meals logged
                    </Typography>
                  </CardContent>
                </Card>
              </Grid>
            ...
            <Card sx={{ 
              ...
              <CardContent>
                ...
                {postureData.recommendations.length > 0 ? (
                  <Box>
                    {postureData.recommendations.slice(0, 4).map((rec, index) => (
